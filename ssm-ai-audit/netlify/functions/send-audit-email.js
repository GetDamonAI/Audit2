exports.handler = async (event) => {
  try {
    ...
    const prioritiesHtml = renderPlainListItems(data.priorities);
    const adminHtml = `
    ...
    const adminSend = await sendOne(
      ...
      `New Audit Lead${businessName ? ` - ${businessName}` : ""}`,
      adminHtml
    );
    const userSend = await sendOne(
      resendKey,
      email,
      `Your AI Visibility Audit${businessName ? ` - ${businessName}` : ""}`,
      userHtml
    );
...
function renderStyledListItems(items) {
  ...
  if (!values.length) {
    return `<li style="margin:0 0 8px;">Not available</li>`;
  }
